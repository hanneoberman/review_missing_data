...
plot_results <- function(results, title_text) {
  ggplot(
    ...
    aes(
      x = versus,
      y = method,
      fill = 100 * count / total,
      label = label#,
      #fontface = ifelse(diag, 2, 1)
    )
  ) +
    geom_tile() +
    ...
    scale_fill_distiller(
      palette = "RdYlGn",
      name = "Comparative performance of the method in the row* (%)",
      na.value = "white",
      direction = 1
    ) + #, low="white", high="darkgrey", na.value = "white") +
    theme_classic() +
    theme(legend.position = "bottom") # "Performance \n(% studies \nin which \nmethod in \nrow > column)
}
...
library(patchwork)
excl_cros <-
  apply(dat[is.na(dat$longitudinal), -c(1:4)], 2, function(x) {
    all(is.na(x))
  }) %>% c(rep(TRUE, 4), .)
excl_long <-
  apply(dat[dat$longitudinal == 1, -c(1:4)], 2, function(x) {
    all(is.na(x))
  }) %>% c(rep(TRUE, 4), .) #dat %>% filter(longitudinal==1) %>% colMeans(., na.rm=TRUE) == "NaN"
